...
- (BOOL)isEmptyString {
    if ([self isKindOfClass:[NSNull class]] || self == nil || [self length] < 1 ? YES : NO) {
        return NO;
    } else {
        return YES;
    }
...
- (CGSize)calculateStringSizeWithFont:(UIFont *)font andSize:(CGSize)size lineSpace:(int)lineSpace {
    ...
    [attributedString1 addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [self length])];
...
+ (float)roundFloat:(float)number {
    NSString *temp = [NSString stringWithFormat:@"%.7f",number];
